...
# ========================
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.where(user: @user_3)[2].items.pluck(:name)
    # ------------------------------------------------------------
...
# ========================
  ...
  it '23. returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    # binding.pry
    # orders = Order.joins(order_items: :id).joins(items: :id).where(id: @item_4)
...
# ========================
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # SELECT DISTINCT items.name FROM items JOIN order_items ON items.id = order_items.item_id ORDER BY items.name;
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # custom_results = []
    # binding.pry
    # custom_results = User.joins(:orders).group(:name).count(:user_id) returns = {"Brian"=>5, "Ian"=>4, "Megan"=>6}
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select("users.*, COUNT(order_items.item_id) as total_item_count")
